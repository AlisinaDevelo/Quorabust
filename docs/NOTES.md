...
- **Holdouts** use connected components of `qid1`/`qid2` when those columns are present,
  preventing question-level leakage between training and evaluation. Datasets without
  complete question IDs use the documented shuffled-row fallback.
...
For audits, Docker, release hygiene, serving, and registry/drift see [ENTERPRISE.md](ENTERPRISE.md). For scale, embeddings, and distributed-training pointers see [SCALING.md](SCALING.md). For Grafana, see [GRAFANA.md](GRAFANA.md).
